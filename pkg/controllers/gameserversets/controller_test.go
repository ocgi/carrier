...
var selectMap = map[string]string{util.GameServerSetGameServerLabel: "test"}
...
func TestControllerSyncGameServerSet(t *testing.T) {
	for _, testCase := range []struct {
		...
		},
		{
			...
			desireReplicas: 2,
			scaleDown:      true,
			ignoreErr:      true, // scale down failed, re-enqueue
		},
		{
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.scaleDown {
				gss.Annotations = map[string]string{util.ScalingReplicasAnnotation: "true"}
			}
	...
	return []*v1alpha1.GameServer{
		{
			...
			}, Labels: map[string]string{util.GameServerSetGameServerLabel: "test"}},
